...
const ExerciseGallery = ({ addExerciseToHEP, isProUser, selectedExercises }) => {
  ...
  const isExerciseInHEP = (exercise) => {
    return selectedExercises.some(
      item => (item.id === exercise.id) || (item._id === exercise._id)
    );
  };
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <main className="w-full md:w-3/4 flex flex-col">
            ...
            {!isProUser && selectedExercises.length === 1 && (
              <div className="bg-gray-800 border-l-4 border-yellow-500 p-3 mb-4 text-sm flex items-center">
                <span className="text-yellow-400 mr-2">ℹ️</span>
                <span className="text-gray-300">Free users can add 1 exercise to HEP. Upgrade to Pro for unlimited exercises.</span>
              </div>
            )}
                  ...
                  {filteredExercises.map((exercise) => (
                    ...
                    >
                      <ExerciseCard
                        ...
                        isInHEP={isExerciseInHEP(exercise)}
