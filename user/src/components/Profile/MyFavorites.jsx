...
import { SquareArrowOutUpRight, Trash2, ChevronLeft, ChevronRight, Heart } from 'lucide-react';
...
const MyFavorites = () => {
    ...
    return (
      <div className="bg-gray-900 p-8 text-center">
        <div className="flex justify-center items-center">
          ...
          <p className="text-primary-400">Loading your favorites...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-gray-900 p-8 text-center">
        ...
        <button onClick={fetchFavorites} className="mt-4 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-md transition">
          Try Again
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-primary-800 rounded-xl px-4 md:px-6 shadow-lg overflow-hidden min-h-screen">
      {/* Page Title */}
      <h1 className="text-xl flex items-center font-bold text-white my-6">
      ...
      <div>
        {favorites.length > 0 ? (
            ...
            {!isMobile && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {favorites.map(favorite => (
                  <div
                    key={favorite._id}
                    className="bg-primary-700/80 rounded-lg overflow-hidden shadow-lg border border-primary-500 transition duration-300"
                  >
                    {/* Media Carousel - Fixed height to match your second image */}
                    {(favorite?.video || favorite.image?.length > 0) && (
                      <div className="h-48 relative bg-primary-900/50">
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <h3
                          className="font-medium flex gap-2 text-white text-lg hover:text-primary-300 cursor-pointer transition"
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4">
                        <div className="bg-primary-800 p-2 rounded-md">
                          <span className="text-primary-300 text-xs font-medium block mb-1">Category</span>
                          <p className="text-white text-sm">{favorite.category}</p>
                        </div>
                        <div className="bg-primary-800 p-2 rounded-md">
                          <span className="text-primary-300 text-xs font-medium block mb-1">Subcategory</span>
                          <p className="text-white text-sm">{favorite.subCategory}</p>
                        </div>
                      </div>

                      {/* Position Badge */}
                      <div className="inline-block bg-primary-600 px-3 py-1 rounded-full text-sm text-white">
                        {favorite.position}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-800 rounded-lg overflow-hidden shadow-lg border border-gray-700 mb-4">
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <h3
                          className="font-medium flex items-center gap-2 text-white text-xl hover:text-primary-300 cursor-pointer transition"
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => handleExerciseClick(favorites[currentIndex]._id)}
                          className="bg-primary-600 hover:bg-primary-700 text-white px-3 py-1 rounded-lg transition-colors"
                        >
                          View
                        </button>
                      </div>
                  ...
                  {favorites.length > 1 && (
                    <div className="flex justify-between">
                      <button
                        ...
                        className="bg-primary-600/20 hover:bg-primary-600/40 text-white p-3 rounded-full transition-colors"
                    ...
                    {favorites.map((_, i) => (
                      <button
                        ...
                        className={`w-2 h-2 rounded-full transition-all duration-300 ${i === currentIndex ?
                          'bg-gradient-to-r from-primary-700 to-primary-500 w-6' :
          ...
          <div className="text-center py-16 bg-gray-800 rounded-lg border border-gray-700">
            <p className="text-primary-400 italic mb-6">You haven't added any favorites yet</p>
            <button onClick={() => navigate('/exercises')} className="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-md transition">
              Browse Exercises
            </button>
          </div>
        )}
